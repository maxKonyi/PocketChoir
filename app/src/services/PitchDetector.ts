...
export class PitchDetector {
  ...
  private settings: PitchDetectionSettings = {
    ...
    maxFrequency: 1000,      // ~B5, high female voice
    smoothingFactor: 0.7,    // How much to smooth (0 = none, 1 = full)
    confidenceThreshold: 0.8, // Minimum confidence to report pitch
  ...
  initialize(stream: MediaStream): void {
    ...
    this.analyser.fftSize = 2048; // Good balance of frequency resolution and speed
  ...
  private detectLoop = (): void => {
    ...
    if (this.pitchTrace.length >= 0 && this.recordingStartTime > 0) {
